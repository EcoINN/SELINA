...
df_tweets <- function(data_path, user = FALSE, verbose = TRUE, output_format = NA) {
...
ls_files <- function(data_path, pattern) {
  ...
  if (length(files) < 1) {
    stop(paste0("There are no files matching the pattern `", pattern, "` in the specified directory."), 
         call. = FALSE)
  }
  return(files)
}
...
convert_json <- function(data_file, output_format = "tidy") {
  ...
  user_data <- .gen_raw(purrr::map_dfr(aux_file, ~jsonlite::read_json(., simplifyVector = TRUE)$users), 
                        pki_name = "author_id")
  names(user_data) <- paste0("user.", names(user_data))
  sourcetweet_data <- list(main = purrr::map_dfr(aux_file, ~jsonlite::read_json(., 
                                                                                simplifyVector = TRUE)$tweets))
...
.second_pass <- function(x, item_name) {
  ...
  if (ncol(dplyr::select_if(x, is.data.frame)) != 0) {
    ...
    return(dplyr::bind_cols(dplyr::select(x, .data$pki), purrr::map2_dfc(ca_df_col, 
                                                                         ca_mother_colnames, .dfcol_to_list)))
  }
...
.simple_unnest <- function(x, pki) {
  if (class(x) == "list" & any(purrr::map_lgl(x, is.data.frame))) {
    tibble::tibble(pki = pki, data = x) %>% dplyr::filter(purrr::map_lgl(.data$data, ~length(.) != 0)) %>% 
      dplyr::group_by(.data$pki) %>% tidyr::unnest(cols = c(.data$data)) %>% dplyr::ungroup() -> res
  } else {
